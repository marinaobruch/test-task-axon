...
export const ProductPage = () => {
	...
	const {
		...
		mode: 'onChange',
		defaultValues: {
			packsNumber: currentProduct.packsNumber,
	...
	const onSubmit: SubmitHandler<IFormFields> = async (data) => {
		const fetchData = () => {
			axios
				.patch(`http://localhost:8081/productTypes/${currentProduct.id}`, {
			...
			>
				...
				<div className='flex justify-center gap-6 mt-5'>
					<button
						...
						className='border border-gray-500 bg-red-500 text-white p-4 rounded-xl hover:bg-red-700'
						type='button'
					>
						Удалить
					</button>
					...
					<button
						...
						type='submit'
					>
						{isSubmitting ? 'В процессе' : 'Сохранить'}
					</button>
				</div>
			</form>
		</div>
